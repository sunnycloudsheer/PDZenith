...
export default class GuestBookingScheduler extends LightningElement {
    ...
    // True when this is the IC first-schedule link path (Lead.IC_Schedule_Link__c)
    // — the family is NOT rescheduling, they're booking for the first time, but
    // we still want the form bypassed since name/email/phone/grade come from
    // the Lead. Apex sets cfg.isLeadFirstSchedule alongside cfg.isRescheduling
    // so the form-skip behavior fires while the "RESCHEDULING YOUR MEETING"
    // banner stays hidden.
    @track _isLeadFirstSchedule = false;
    ...
    _action = '';
    _rescheduleToken = '';
    // Captured at the moment the family lands on the booking page (in
    // connectedCallback) so it always reflects the original landing URL,
    // even if the LWC later modifies query params or the browser history.
    // Trimmed to 255 chars because ServiceAppointment.Landing_Page_Source__c
    // is a URL field with the standard 255-char cap.
    _landingPageSource = '';
    ...
    connectedCallback() {
        ...
        // Capture the full landing-page URL the family arrived on so we can
        // stamp it onto SA.Landing_Page_Source__c later. window.location.href
        // includes the booking-site host + path + token + UTM params, which
        // is what reporting wants. Trimmed to the 255-char field cap.
        try {
            const href = (typeof window !== 'undefined' && window.location && window.location.href)
                ? window.location.href : '';
            this._landingPageSource = href.length > 255 ? href.substring(0, 255) : href;
        } catch (urlErr) {
            this._landingPageSource = '';
        }

        this._action = (this._getUrlParam('action') || '').toLowerCase();
        const urlToken = this._getUrlParam('token') || '';

        // action=schedule is the IC-from-Lead first-schedule flow (link is on
        // Lead.IC_Schedule_Link__c, generated by Lead_Generate_IC_Schedule_Token).
        // Routed through _rescheduleToken so the existing "skip Your Info form +
        // pre-fill from server" path handles it — Apex resolves which token type
        // it is (SA reschedule / Lead rebook / Lead first-schedule) and returns
        // rescheduleContactInfo accordingly.
        if ((this._action === 'reschedule' || this._action === 'cancel'
             || this._action === 'schedule') && urlToken) {
    ...
    // Reschedule banner is for genuine reschedules + Lead post-cancel rebooks
    // only — NOT for first-schedule links from a Lead's IC_Schedule_Link__c
    // (the family is booking their FIRST meeting, so showing "RESCHEDULING
    // YOUR MEETING" would be wrong copy).
    get showRescheduleBanner() {
        return this.isRescheduling && !this._isLeadFirstSchedule;
    }
    get headerLabel() {
    ...
    async _init() {
        try {
            ...
            if (cfg.isRescheduling) {
                ...
                this._isLeadFirstSchedule = !!cfg.isLeadFirstSchedule;
    ...
    async handleSubmit() {
        ...
        try {
            const result = await bookAppointment({
                ...
                rescheduleToken: this._rescheduleToken || null,
                notes: '',
                landingPageSource: this._landingPageSource || ''
            });
